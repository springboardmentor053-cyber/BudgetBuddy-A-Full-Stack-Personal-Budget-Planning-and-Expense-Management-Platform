...
class FinancialSummaryAPIView(APIView):
    ...
    def get(self, request):
        ...
        total_income = float(
            Income.objects.filter(user=user).aggregate(
        ...
        total_budget = float(
            Budget.objects.filter(user=user).aggregate(
                total=Coalesce(
                    Sum("monthly_limit", output_field=FloatField()), 0.0)
            )["total"]
        )
        total_savings = float(
            SavingsGoal.objects.filter(user=user).aggregate(
                total=Coalesce(
...
class CategoryAnalysisAPIView(APIView):
    ...
    def get(self, request):
        category_data = (
            ...
            .values("category")
            .annotate(total_spending=Coalesce(Sum("amount", output_field=FloatField()), 0.0))
            .order_by("category")
        )
...
class MonthlyExpenseTrendAPIView(APIView):
    ...
    def get(self, request):
        monthly_data = (
            ...
            .values("month")
            .annotate(total_expense=Coalesce(Sum("amount", output_field=FloatField()), 0.0))
            .order_by("month")
        )
...
class DashboardAPIView(APIView):
    ...
    def get(self, request):
        ...
        budget_usage_percentage = (
            round((total_expense / total_budget * 100),
                  1) if total_budget > 0 else 0.0
        )
        ...
        category_qs = (
            ...
            .values("category")
            .annotate(amount=Coalesce(Sum("amount", output_field=FloatField()), 0.0))
            .order_by("category")
        )
        ...
        for cat in category_qs:
            ...
            if amount > 0:
                pct = round((amount / total_expense * 100),
                            1) if total_expense > 0 else 0.0
        ...
        income_by_month = Income.objects.filter(user=user).annotate(
            m=ExtractMonth("income_date")
        ).values("m").annotate(total=Sum("amount"))

        expense_by_month = Expense.objects.filter(user=user).annotate(
            m=ExtractMonth("expense_date")
        ).values("m").annotate(total=Sum("amount"))

        inc_map = {item["m"]: float(item["total"])
                   for item in income_by_month if item["m"]}
        exp_map = {item["m"]: float(item["total"])
                   for item in expense_by_month if item["m"]}
        ...
        active_savings_goals = list(
            SavingsGoal.objects.filter(user=user, status="ACTIVE")
            .values(
                "id",
                "goal_name",
                "goal_type",

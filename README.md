# BudgetBuddy 💰

### A Full-Stack Personal Budget Planning and Expense Management Platform
BudgetBuddy is a personal finance management platform designed to help users (especially students) manage pocket money, track expenses, set savings goals, monitor spending habits, and generate financial reports for better financial discipline[cite: 1].

---

## 🚀 Project Status: Milestone Tracker

*   **Milestone 1 (Requirements, Database Design & Backend Setup):**  **Completed**[cite: 1]  
    *   Defined project scope and database schema[cite: 1].
    *   Initialized backend skeleton and configured JWT authentication[cite: 1].
    *   Created React frontend skeleton[cite: 1].
*   **Milestone 2 (Expense & Income Management):**  **In Progress**[cite: 1]  
    *   **Expense Module:**  **Completed** (Successfully implemented expense tracking, categorization, and transaction logs)[cite: 1].
    *   **Income Management & Budget Planning:** ⏳ **Pending**[cite: 1].
*   **Milestone 3 (Savings Goals & Notifications):** ⏳ **Pending**[cite: 1].
*   **Milestone 4 (Analytics, Testing & Deployment):** ⏳ **Pending**[cite: 1].

---

## 🛠️ Tech Stack

*   **Frontend:** React.js, Tailwind CSS, Axios, React Router[cite: 1]
*   **Backend:** Python, Django REST Framework / FastAPI[cite: 1]
*   **Database:** SQLite (Local Dev) / PostgreSQL (Production)[cite: 1]
*   **Authentication:** JWT (JSON Web Tokens)[cite: 1]

---

## 📦 Features & Modules

### 1. Completed Features
*   **User Authentication:** Secure JWT-based login and registration flow[cite: 1].
*   **Expense Tracking System:**[cite: 1]
    *   Add, edit, and delete expenses[cite: 1].
    *   Expense categorization (Food, Travel, Shopping, Education, Entertainment, Miscellaneous)[cite: 1].
    *   Daily transaction history log[cite: 1].

### 2. Pending Features (To Be Implemented)
*   **Income Management:** Tracking pocket money, scholarships, and freelance earnings[cite: 1].
*   **Budget Planning:** Category-wise monthly budget allocation and limit alerts[cite: 1].
*   **Savings Goals:** Goal creation and visual progress tracking (e.g., Laptop Fund, Emergency Fund)[cite: 1].
*   **Analytics Dashboard:** Interactive charts showing monthly trends and category-wise spending insights[cite: 1].
*   **Notification System:** Automated alerts for budget limit breaches and savings reminders[cite: 1].
*   **Reports & Export:** Export financial summaries and expense history to PDF/Excel[cite: 1].

---

